""" Two Convolutions Per Level Network Architecture
...
class DefineDoseFromCT:
    ...
    def define_generator(self) -> Model:
        ...
        x5 = self.make_convolution_block(x4a, 8 * self.initial_number_of_filters)
        x5a = self.make_convolution_block(x5, 8 * self.initial_number_of_filters, strides=(1,1,1))
        x6 = self.make_convolution_block(x5a, 8 * self.initial_number_of_filters, use_batch_norm=False)
        x6a = self.make_convolution_block(x6, 8 * self.initial_number_of_filters, use_batch_norm=False, strides=(1,1,1))

        # Build model back up from bottleneck
        x5b = self.make_convolution_transpose_block(x6a, 8 * self.initial_number_of_filters, use_dropout=False)
        x5bc = self.make_convolution_block(x5b, 8 * self.initial_number_of_filters, strides=(1,1,1))
